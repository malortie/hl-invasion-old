...
void EV_FireSG3(struct event_args_s *args);
void EV_LFlammes(struct event_args_s *args);
void EV_RpgIdle(struct event_args_s *args);
void EV_RpgReload(struct event_args_s *args);
#endif // defined ( HLINVASION_CLIENT_DLL )
}
...
void EV_FireShotGunDouble( event_args_t *args )
{
	...
	{
#if defined ( HLINVASION_CLIENT_DLL )
		EV_HLDM_FireBullets(idx, forward, right, up, 8, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT_DOUBLE, 0, &tracerCount[idx - 1], VECTOR_CONE_20DEGREES.x, VECTOR_CONE_5DEGREES.y);
#else
	...
	{
#if defined ( HLINVASION_CLIENT_DLL )
		EV_HLDM_FireBullets(idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT_DOUBLE, 0, &tracerCount[idx - 1], VECTOR_CONE_10DEGREES.x, VECTOR_CONE_10DEGREES.y);
#else
...
void EV_RpgIdle(event_args_t *args)
{
	...
	//Only play the weapon anims if I am the local client. 
	if (EV_IsLocal(idx))
	{
...
void EV_RpgReload(event_args_t *args)
{
	int idx;
	int anim, body;
...
void EV_FireM16(event_args_t *args)
{
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/m16_shoot_1.wav", 1, ATTN_NORM, 0, 95);
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/m16_shoot_2.wav", 1, ATTN_NORM, 0, 95);
		break;
	}
...
void EV_FireFSniper(event_args_t *args)
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/fsniper_shoot1.wav", 1, ATTN_NORM, 1, PITCH_NORM);
...
void EV_FireSG(event_args_t *args)
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(SG_SHOOT1, 2);
		V_PunchAxis(0, gEngfuncs.pfnRandomFloat(8, 10));
	}

	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "turret/tu_fire1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
}
...
void EV_FireSG2(event_args_t *args)
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(SG_BIGSHOOT, 2);
	}

	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/gauss2.wav", 1, ATTN_NORM, 0, PITCH_NORM);
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_ITEM, "debris/beamstart14.wav", 1, ATTN_NORM, 0, PITCH_NORM);
}

void EV_FireSG3(event_args_t *args)
{
...
void EV_LFlammes(event_args_t *args)
{
	int idx;
	int mode;
	...
	if (EV_IsLocal(idx))
	{
		switch (mode)
		{
		case 0: gEngfuncs.pEventAPI->EV_WeaponAnimation(LFLAMMES_CLOSE, 2);	break;
		case 1: gEngfuncs.pEventAPI->EV_WeaponAnimation(LFLAMMES_OPEN, 2); break;
		case 2: gEngfuncs.pEventAPI->EV_WeaponAnimation(LFLAMMES_FIRE, 2); break;
		default: break;
		}
	}
}
...
void EV_FireIRgun(event_args_t *args)
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex("models/beretta_shell.mdl");// brass shell

	if (EV_IsLocal(idx))
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(empty ? IRGUN_FIRE_EMPTY : IRGUN_FIRE1, 2);
	...
	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_IRGUN, 0, 0, args->fparam1, args->fparam2);
}
