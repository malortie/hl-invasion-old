...
BOOL CIRgun::Deploy()
{
	return DefaultDeploy("models/v_irgun.mdl", "models/p_357.mdl", IRGUN_DRAW, "irgun", UseDecrement());
}
...
void CIRgun::WeaponIdle(void)
{
	...
	{
		iAnim = IRGUN_IDLE_IR; // idle_ir
		m_flTimeWeaponIdle = UTIL_SharedRandomFloat(m_pPlayer->random_seed, 3, 6);
	}
	else if (flRand <= 0.7)
	{
