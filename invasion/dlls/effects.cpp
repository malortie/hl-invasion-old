...
void CMessage::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// HL: Invasion
	//
	// Remove an unspecified credit message.
	//
	if (FStrEq(STRING(gpGlobals->mapname), "l5m5") && FStrEq(STRING(pev->message), "END_TITLE18bis"))
	{
		UTIL_Remove(this);
		return;
	}
	//
	// Remove unspecified chapter title.
	//
	if (FStrEq(STRING(gpGlobals->mapname), "l5m1") && FStrEq(STRING(pev->message), "l5m1_title"))
	{
		UTIL_Remove(this);
		return;
	}
